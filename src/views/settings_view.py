...
def build_settings_view(
    ...
    async def _on_gpu_change(e):
        ...
        if storage:
            await storage.set(STORAGE_DEFAULT_GPU, state.default_gpu)

    async def _on_tpu_change(e):
        state.default_tpu = e.control.value or ""
        if storage:
            await storage.set(STORAGE_DEFAULT_TPU, state.default_tpu)

    async def _on_timeout_change(e):
        state.default_timeout = int(e.control.value)
        if storage:
            await storage.set(STORAGE_DEFAULT_TIMEOUT, state.default_timeout)

    async def _on_keep_alive_change(e):
        state.keep_alive_enabled = e.control.value
        if storage:
            await storage.set(STORAGE_KEEP_ALIVE, state.keep_alive_enabled)
    ...
    async def on_clear_data(e):
        ...
        dialog = ft.AlertDialog(
            ...
            content=ft.Text(
                ...
                "Your Cloud sessions and Google account will not be affected."
            ),
            actions=[
    ...
    # ── Debug terminal ──────────────────────────────────────────
    debug_output = ft.Ref[ft.Text]()

    async def _run_debug(e):
        if not colab_service or not state.active_session_name:
            if debug_output.current:
                debug_output.current.value = (
                    "No active session. Start one from the Notebook tab."
                )
                debug_output.current.visible = True
                page.update()
            return

        if debug_output.current:
            debug_output.current.value = "Running diagnostics..."
            debug_output.current.visible = True
            page.update()
        ...
        try:
            ...
            if debug_output.current:
                debug_output.current.value = "".join(text_parts) or "No output"
        except Exception as ex:
            if debug_output.current:
                debug_output.current.value = f"Error: {ex}"
        page.update()
    ...
    content = ft.Column(
        controls=[
            ...
            setting_tile(
                ...
                subtitle="Cloud OAuth2 — tap to verify"
            ...
            *(
                            ...
                            controls=[
                                ...
                                ft.Column(
                                    [
                                        ...
                                        ft.Text(
                                            "Run diagnostics on the active Cloud session",
                                            size=tokens.FONT_XS,
                                            color=ft.Colors.ON_SURFACE_VARIANT,
                                            italic=True,
                                        ),
            ...
            setting_tile(
                ...
                subtitle="Reset preferences (Cloud account unaffected)",
                    ...
                    controls=[
                        ...
                        ft.Row(
                            [
                                ...
                                ft.Text(
                                    f"Google Cloud (CLI v{_cli_version})",
                                    size=tokens.FONT_SM,
                                    color=ft.Colors.ON_SURFACE_VARIANT,
                                ),
                            ],
