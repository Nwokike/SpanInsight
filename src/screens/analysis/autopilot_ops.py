...
async def run_autopilot_async(
    ...
    history = []
    run_cells = []  # cells this run produced successfully -> auto-pinned to the report
    try:
    ...
    # Pin only the cells this run produced (legacy parity), transforming each into
    # the canonical report-block shape the renderer + sharer consume.
    if run_cells:
        try:
            from services.report_service import (
                ReportService,
                build_report_block_from_cell,
            )
            ...
            report_blocks = [build_report_block_from_cell(c) for c in run_cells]
            ...
            state.autopilot_steps.append(
                {
                    "text": f"Executive report compiled ({len(report_blocks)} insights)",
                    "status": "done",
